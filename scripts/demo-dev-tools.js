...
class DevToolsDemo {
  ...
  demoJq() {
    ...
    if (fs.existsSync('collected-components/components-metadata.json')) {
      ...
      const componentCount = this.execCommand(
        'jq "length" collected-components/components-metadata.json'
      );
      console.log(`Всего компонентов: ${componentCount}`);

      const categories = this.execCommand(
        'jq "group_by(.category) | map({category: .[0].category, count: length})" collected-components/components-metadata.json'
      );
...
`;
  ...
  demoAstGrep() {
    ...
    const reactComponents = this.execCommand(
      'ast-grep --pattern "const $COMPONENT = () => { $$$ }" --lang tsx src/'
    );
    ...
    const exports = this.execCommand(
      'ast-grep --pattern "export const $NAME" --lang tsx --lang ts src/'
    );
    ...
    const interfaces = this.execCommand(
      'ast-grep --pattern "interface $NAME { $$$ }" --lang tsx --lang ts src/'
    );
  ...
  run() {
    ...
    console.log(
      '  ast-grep --pattern "const $NAME = () => { $$$ }" --lang tsx  # Найти компоненты'
    );
  }
}
